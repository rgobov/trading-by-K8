    ...
    def run_for_candidates(self, candidates: pd.DataFrame, years: int = 3,
        ...
        for day in all_dates:
            ...
            for t in sorted_ops:
                ...
                if buy_price is None or sell_price is None or np.isnan(buy_price) or np.isnan(sell_price):
                    continue
                ...
                if shares < 1:
                    continue
                if not self.compounding and int(self.initial_capital * t["pos_frac"] / buy_price) < 1:
                    continue
